...
def render_sidebar(dfs: dict, progresses: dict, datasets: dict) -> None:
    ...
    if st.sidebar.button("Training · 5 Answers", use_container_width=True):
        st.session_state.app_mode = "train5"
        st.rerun()

    if st.sidebar.button("Self-test", use_container_width=True):
...
def render_training_mode(dataset_key: str, df: pd.DataFrame, progress: dict, datasets: dict) -> None:
    ...
    if not active_queue and st.session_state[keys["screen_mode"]] == "question":
        st.warning("This list is empty. Please choose another list.")
        st.stop()
    ...
    st.caption(f"Questions in this list: {len(active_queue)}")
    ...
    if st.session_state[keys["screen_mode"]] == "question":
        ...
        if result["is_correct"]:
            ...
            st.error(
                f"Incorrect. The question was moved to {BUCKET_LABELS[result['moved_to_bucket']]} "
                "and will appear again soon."
            )
        ...
        st.write(
            f"**Your answer:** "
            f"{', '.join(selected_letters) if selected_letters else 'No answer selected'}"
        )
        ...
        if st.button(
            ...
            key=f"{dataset_key}_next_{current_qid}",
        ):
            updated_progress = load_or_create_progress(df, config["progress_path"])
            next_qid = pick_current_question(
                updated_progress,
                st.session_state[keys["active_bucket"]],
            )
...
def start_test(dfs: dict, n_questions: int) -> None:
    ...
    if not pool:
        st.error("No questions found for the test.")
        return
...
def render_selftest_mode(dfs: dict, progresses: dict, datasets: dict) -> None:
    ...
    if not st.session_state.test_started:
        value = st.number_input(
            "How many questions do you want in the test?",
            min_value=1,
    ...
    if st.session_state.test_finished:
        ...
        st.write(f"**Correct:** {correct} of {total}")
        st.write(f"**Incorrect:** {total - correct} of {total}")
        st.info(
            "Only incorrectly answered questions were moved to List 0 of their respective question set "
    ...
    st.caption(f"Test question {current_index + 1} of {total_questions} · Source: {config['label']}")
    render_question_text(row, q_type)
    ...
    if not st.session_state.test_show_feedback:
        ...
        if st.button(
            ...
            type="primary",
            use_container_width=True,
            key=f"test_submit_{current_index}",
        ):
            selected_set = set(selected_indices)
            is_correct = selected_set == correct_set
        ...
        button_label = "Finish test" if current_index == total_questions - 1 else "Next test question"

        if st.button(
            button_label,
            type="primary",
            use_container_width=True,
            key=f"test_next_{current_index}",
        ):
